...
import fs from 'fs';
const tiktokUsername = '@irlandagl1'; // replace with your TikTok username
...
const keywordTogive = {
    ...
    'azucar': 'minecraft:sugar',
    'azucar_comestible': 'minecraft:cake',
    'azucar_de_caña': 'minecraft:sugar_cane',
    'azucar_de_calabaza': 'minecraft:pumpkin_seeds',
    'azucar_de_melón': 'minecraft:melon_seeds',
    'azucar_de_pasto': 'minecraft:wheat_seeds',
    'azucar_de_trigo': 'minecraft:wheat_seeds',
    'azucar_de_zanahoria': 'minecraft:carrot',
    'azucar_de_patata': 'minecraft:potato',
    'azucar_de_remolacha': 'minecraft:beetroot_seeds',
    'azucar_de_cacao': 'minecraft:cocoa_beans',
    'azucar_de_alga': 'minecraft:kelp',
    ...
    'caña_de_azucar': 'minecraft:sugar_cane',
...
function handleEvent(eventType, data) {
    ...
    if (commandList[eventType]) {
        ...
        eventCommands.forEach(command => {
            ...
            if (command.includes('give @a minecraft:{item}') && keywordTogive[data.comment.toLowerCase()]) {
                let itemKeyword = Object.keys(keywordTogive).find(keyword => data.comment.toLowerCase().includes(keyword.toLowerCase()));
                if (itemKeyword) {
                    replacedCommand = `execute at ${playername} run give ${playername} minecraft:${keywordTogive[itemKeyword]}`;
            ...
            if (command.includes('summon {mob}') && keywordTomob[data.comment.toLowerCase()]) {
                let mobKeyword = Object.keys(keywordTomob).find(keyword => data.comment.toLowerCase().includes(keyword.toLowerCase()));
                if (mobKeyword) {
                    replacedCommand = `execute at ${playername} run summon ${keywordTomob[mobKeyword]}`;
                    console.log(replacedCommand);
                }
        ...
        for (const [key, value] of Object.entries(commandList.scoreboard)) {
            ...
            if (value <= 50) {
                ...
                setTimeout(() => {
                    sendCommand(`/title @a actionbar [{"text":"${key}: "}, {"text":"${value}"}]`);
                }, delay);
